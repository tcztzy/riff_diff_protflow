...
from protflow.config import RFDIFFUSION_SCRIPT_PATH
...
def write_partial_rfdiffusion_opts(poses: Poses, length: int, preserve_col: str, channel_contig_col: str = None) -> list:
    cmds = []
    for _, row in poses.df.iterrows():
        contigmap_contigs = determine_partial_diff_contig(length=length, motif=row[preserve_col])
        if channel_contig_col:
            contigmap_contigs = contigmap_contigs + " " + row[channel_contig_col]
        contigmap_provide_seq = determine_partial_diff_seq(motif=row[preserve_col])
        cmds.append(f"'contigmap.contigs=[{contigmap_contigs}]' 'contigmap.provide_seq=[{contigmap_provide_seq}]'")
    return cmds

def determine_partial_diff_contig(length: int, motif: ResidueSelection) -> str:
    # create a dict to access resnums directly
    motif_dict = motif.to_dict()
    sorted_residues = sorted(motif_dict["A"])
    # group contigous stretches in sublists
    residues_list = find_contiguous_stretches(sorted_residues)
    # identify min and max in each sublist
    residues_list = [[min(stretch), max(stretch)] for stretch in residues_list]
    # calculate differences between max from list n and min from list n+1
    differences = []
    for i in range(len(residues_list) -1):
        differences.append(residues_list[i+1][0] - residues_list[i][1] - 1)
    # determine start patch
    start = residues_list[0][0] - 1
    #  determine end patch
    end = length - residues_list[-1][-1]
    # create patches
    residues_list = [f"A{stretch[0]}-{stretch[1]}" for stretch in residues_list]
    # combine start and end patches with inbetween patches
    differences = [start] + differences + [end]
    # create list of alternating patches and preserved motifs
    interleaved = []
    for i in range(len(residues_list)):
        interleaved.append(str(differences[i]))
        interleaved.append(residues_list[i])
    interleaved.append(str(differences[-1]))
    
    return "/".join(interleaved)

def determine_partial_diff_seq(motif):
    # create a dict to access resnums directly
    motif_dict = motif.to_dict()
    sorted_residues = sorted(motif_dict["A"])
    # group contigous stretches in sublists
    residues_list = find_contiguous_stretches(sorted_residues)
    # seq contigs are 0-indexed
    residues_list = [f"{min(stretch)-1}-{max(stretch)-1}" for stretch in residues_list]
    return ",".join(residues_list)

def find_contiguous_stretches(numbers):
    ...
    for i in range(1, len(numbers)):
        ...
        if numbers[i] == numbers[i - 1] + 1:
            ...
            # If not, save the current stretch and start a new one
            contiguous_stretches.append(current_stretch)
            current_stretch = [numbers[i]]

    # Append the final stretch
    contiguous_stretches.append(current_stretch)
    
    return contiguous_stretches

def replace_motif(poses: Poses, motif_sel_col: ResidueSelection, motif_path_col: str, output_dir: str) -> Poses:
    df = []
    os.makedirs(output_dir, exist_ok=True)
    for _, row in poses.df.iterrows():
        pose = load_structure_from_pdbfile(row["poses"])
        motif_pose = load_structure_from_pdbfile(row[motif_path_col])
        pose_chain = pose["A"]
        for chain, resnum in row[motif_sel_col]:
            res = pose[chain][resnum]
            motif_res = motif_pose[chain][resnum]
            motif_res.id = res.id
            motif_res.resname = res.get_resname()
            pose_chain.detach_child(res.id)
            pose_chain.insert(resnum-1, motif_res)
        save_structure_to_pdbfile(pose, pose_path := os.path.join(output_dir, os.path.basename(row["poses"])))
        row["poses"] = pose_path
        df.append(row)
    poses.df = pd.DataFrame(df)
    return poses
...
def main(args):
    ...
    if args.screen_input_json:
        ...
        rfdiffusion_dir = os.path.dirname(os.path.dirname(RFDIFFUSION_SCRIPT_PATH))
        partial_diffusion_opts = f"diffuser.partial_T={args.screen_partial_diffusion_timesteps} inference.ckpt_override_path={os.path.join(rfdiffusion_dir, 'models', 'ActiveSite_ckpt.pt')}"
        ...
        for prefix, setting in zip(prefixes, settings):
            ...
            backbones.df["partial_diff_pose_opts"] = write_partial_rfdiffusion_opts(poses=backbones, length=args.total_length, preserve_col="motif_residues", channel_contig_col="channel_contig" if "channel_contig" in backbones.df.columns else None)

            # exchange motif residues with original motif
            logging.info("Resetting to original motif...")
            backbones = replace_motif(poses=backbones, motif_sel_col="motif_residues", motif_path_col="updated_reference_frags_location", output_dir=os.path.join(backbones.work_dir, "motif_replacement"))
            
            if "channel_contig" in backbones.df.columns:
                logging.info("Adding channel placeholder...")
                chain_adder.add_chain(
                poses = backbones,
                prefix = f"readd_channel_partialdiff",
                ref_col = "input_poses",
                copy_chain = "Q")
            
            # run partial rfdiffusion
            logging.info("Optimizing active site motif with partial RFdiffusion...")
            rfdiffusion.run(
                poses=backbones, 
                prefix=f"partial_rfdiffusion", 
                num_diffusions=args.screen_partial_diff_num_diffusions,
                options=partial_diffusion_opts,
                pose_options="partial_diff_pose_opts")

            # remove channel chain (chain B)
            logging.info(f"Partial diffusion completed, removing channel chain from diffusion outputs.")
            if "channel_contig" in backbones.df.columns:
                chain_remover.run(
                    poses = backbones,
                    prefix = "channel_removed_partialdiff",
                    chains = "B"
                )
            else:
                backbones.df["channel_removed_location"] = backbones.df["partial_rfdiffusion_location"]

            logging.info(f"Adding back ligand post-diffusion...")
            chain_adder.superimpose_add_chain(
                poses = backbones,
                prefix = f"postpartialdiffusion_ligand",
                ref_col = "updated_reference_frags_location",
            ...
            if args.screen_skip_mpnn_rlx_mpnn:
                ...
                # filter backbones down to rfdiffusion backbones (per backbone filtering)
                backbones.filter_poses_by_rank(n=args.screen_num_mpnn_sequences, score_col="pre_esm_comp_score", prefix="esm_input_filter_per_bb", plot=True, plot_cols=["bbopt_total_score", "bbopt_lig_contacts", "bbopt_ligand_clashes", "rfdiffusion_rog_data", "mpnn_overall_confidence"], remove_layers=2)
                ...
                # filter backbones down to rfdiffusion backbones (per backbone filtering)
                backbones.filter_poses_by_rank(n=args.screen_num_mpnn_sequences, score_col="pre_esm_comp_score", prefix="esm_input_filter_per_bb", plot=True, plot_cols=["bbopt_total_score", "bbopt_lig_contacts", "bbopt_ligand_clashes", "rfdiffusion_rog_data", "mpnn_overall_confidence"], remove_layers=4)
    ...
    if args.ref_input_json:
        ...
        if args.ref_input_poses_per_bb:
            logging.info(f"Filtering refinement input poses on per backbone level according to screen_composite_score...")
            backbones.filter_poses_by_rank(n=args.ref_input_poses_per_bb, score_col=f'screen_composite_score', remove_layers=1, prefix='refinement_input_bb', plot=True, plot_cols=["esm_plddt", "esm_tm_TM_score_ref", "esm_catres_bb_rmsd", "esm_catres_heavy_rmsd", "esm_lig_contacts", "esm_ligand_clashes", "esm_rog_data"])
        if args.ref_input_poses:
            logging.info(f"Filtering refinement input according to screen_composite_score...")
            backbones.filter_poses_by_rank(n=args.ref_input_poses, score_col=f'screen_composite_score', prefix='refinement_input', plot=True, plot_cols=["esm_plddt", "esm_tm_TM_score_ref", "esm_catres_bb_rmsd", "esm_catres_heavy_rmsd", "esm_lig_contacts", "esm_ligand_clashes", "esm_rog_data"])
        ...
        for cycle in range(args.ref_start_cycle, args.ref_cycles+1):
            ...
            backbones.filter_poses_by_value(score_col=f"cycle_{cycle}_motif_rmsd", value=bb_rmsd_cutoff, operator="<=", prefix=f"cycle_{cycle}_motif_bb", plot=True)
            ...
            backbones.calculate_composite_score(
                ...
                scoreterms=[f"cycle_{cycle}_esm_plddt", f"cycle_{cycle}_esm_tm_TM_score_ref", f"cycle_{cycle}_esm_catres_bb_rmsd", f"cycle_{cycle}_esm_catres_heavy_rmsd", f"cycle_{cycle}_motif_rmsd", f"cycle_{cycle}_delta_apo_holo", f"cycle_{cycle}_postrelax_ligand_rmsd", f"cycle_{cycle}_postrelax_catres_heavy_rmsd", f"cycle_{cycle}_fastrelax_sap_score_mean", f"cycle_{cycle}_postrelax_apo_catres_heavy_rmsd"],
                weights=[-2, -2, 8, 4, 2, 2, 2, 2, 1, 2],
                plot=True
            )
...
if __name__ == "__main__":
    ...
    argparser.add_argument("--screen_decentralize_weights", type=str, default="40", help="Decentralize weights that should be tested during screening. Separated by ','.")
    argparser.add_argument("--screen_decentralize_distances", type=str, default="2", help="Decentralize distances that should be tested during screening. Separated by ','.")
    ...
    argparser.add_argument("--screen_partial_diffusion_timesteps", type=int, default=10, help="Timesteps for partial diffusion")
    argparser.add_argument("--screen_partial_diff_channel_placeholder", action="store_true", help="Readd channel placeholder for partial diffusion steps")
    argparser.add_argument("--screen_partial_diff_num_diffusions", type=int, default=15, help="Readd channel placeholder for partial diffusion steps")
    argparser.add_argument("--screen_rfdiffusion_output_per_bb", type=int, default=5, help="Readd channel placeholder for partial diffusion steps")
    ...
    argparser.add_argument("--rfdiffusion_catres_bb_rmsd", type=float, default=0.1, help="Filter RFdiffusion output for catalytic residue backbone rmsd.")
    argparser.add_argument("--rfdiffusion_motif_bb_rmsd", type=float, default=0.1, help="Filter RFdiffusion output for fragment motif backbone rmsd.")
