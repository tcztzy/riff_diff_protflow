...
def pool_all_cycle_results(ref_dict: dict, plddt_cutoff:float, catres_bb_rmsd_cutoff:float, motif_bb_rmsd_cutoff:float, ligand_rmsd_cutoff:float, comp_score_scoreterms:list, comp_score_weights:list, refinement_dir:str) -> Poses:
    ...
    for cycle in ref_dict:
        ...
        backbones.filter_poses_by_value(score_col="cycle_final_placer_rmsd_weighted", value=ligand_rmsd_cutoff, operator="<=", fail_on_empty=False)
...
def main(args):
    ...
    placer = protflow.tools.placer.PLACER(jobstarter=real_gpu_jobstarter)
    ...
    if params:
        ...
        hbond_options = hbond_options +  f" -extra_res_fa {' '.join(params)}"
    ...
    if args.ref_input_json:
        ...
        # instantiate plotting trajectories
        # TODO: fix plotting trajectory_plots = instantiate_trajectory_plotting(backbones.plots_dir, backbones.df)
        ...
        for cycle in range(args.ref_start_cycle, args.ref_cycles+1):
            ...
            esm_comp_scoreterms = [f"cycle_{cycle}_esm_plddt", f"cycle_{cycle}_esm_catres_bb_rmsd", f"cycle_{cycle}_esm_catres_heavy_rmsd", f"cycle_{cycle}_esm_motif_rmsd", f"cycle_{cycle}_esm_contacts_score", f"cycle_{cycle}_esm_ligand_clashes"]
            esm_comp_weights = [-2, 6, 3, 1, 2, 2]
            backbones.calculate_composite_score(
                name=f"cycle_{cycle}_post_esm_composite_score",
                scoreterms=esm_comp_scoreterms,
                weights=esm_comp_weights,
                plot=True
            )

            # define number of index layers added during refinement so far
            layers = 4 if args.attnpacker_repack else 3

            # filter down to refinement input poses
            logging.info("Filtering poses according to composite score...")
            backbones.filter_poses_by_rank(
                n=5,
                score_col=f"cycle_{cycle}_post_esm_composite_score",
                prefix=f"cycle_{cycle}_post_esm_composite_score",
                plot=True,
                plot_cols=esm_comp_scoreterms,
            ...
            backbones.filter_poses_by_value(score_col=f"cycle_{cycle}_placer_rmsd_weighted", value=ligand_rmsd_cutoff, operator="<=", prefix=f"cycle_{cycle}_ligand_rmsd", plot=True)        
            ...
            ref_comp_scoreterms = [f"cycle_{cycle}_esm_plddt", f"cycle_{cycle}_esm_catres_bb_rmsd", f"cycle_{cycle}_esm_catres_heavy_rmsd", f"cycle_{cycle}_esm_motif_rmsd", f"cycle_{cycle}_placer_rmsd_weighted", f"cycle_{cycle}_placer_kabsch_weighted", f"cycle_{cycle}_placer_hbonds_weighted", f"cycle_{cycle}_placer_catres_heavy_rmsd_weighted", f"cycle_{cycle}_esm_contacts_score", f"cycle_{cycle}_placer_prmsd_top_0.1_avg", f"cycle_{cycle}_esm_ligand_clashes"]
            ref_comp_weights = [-2, 2, 1, 1, 4, 2, -2, 2, 1, 4, 1]
            backbones.calculate_composite_score(
    ...
    if args.eval_input_json:
        ...
        if last_ref_cycle == "final":
            ...
            for cycle in range(1, last_ref_cycle + 1):
                trajectory_plots = update_trajectory_plotting(trajectory_plots, backbones.df, cycle)
        trajectory_plots = add_eval_data_to_trajectory_plots(backbones.df, trajectory_plots)
        """
    ...
    if args.variants_input_json:
        ...
        # add covalent bonds info to poses pre-relax
        backbones = add_covalent_bonds_info(poses=backbones, prefix="variants_esm_cov_info", covalent_bonds_col="covalent_bonds")
...
if __name__ == "__main__":
    ...
    argparser.add_argument("--screen_decentralize_weights", type=str, default="20", help="Decentralize weights that should be tested during screening. Separated by ','.")
