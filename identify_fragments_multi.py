...
# import dependencies
import Bio
from Bio.PDB import Structure, Model, Chain, Residue, Atom, vectors
import math
import pandas as pd
import numpy as np
...
def update_covalent_bonds(covalent_bonds:str, rotamer_id:str, rotamer_position:int, ligand_dict:dict):
    ...
    for cov_bond in covalent_bonds.split(","):
        res_data, lig_data = cov_bond.split(":")
        res_data, res_atom = res_data.split("-")
        lig_data, lig_atom = lig_data.split("-")
        lig_resnum, lig_chain = split_pdb_numbering(lig_data)
...
def run_matcher(cst_files, lig_name, output_dir, match_application, database_dir, jobstarter, params_path=None):
    fragment_df = pd.read_json(os.path.join(database_dir, "matching", "fragments.json"))
    fragment_paths = fragment_df["path"].to_list()

    # generate overall options
    match_flags = [
        f"-match:lig_name {lig_name}",
        f"-extra_res_fa {params_path}",
        f"-match:grid_boundary {os.path.join(database_dir, 'matching', 'grid.gridlig')}",
        f"-match:scaffold_active_site_residues_for_geomcsts {os.path.join(database_dir, 'matching', 'pos.posfile')}",
        "-run:preserve_header",
        ...
        "-ex2",
        "-extrachi_cutoff 0"]
    if params_path: match_flags.append(f"-extra_res_fa {params_path}")
    # create every combination of fragments and cst files
    frag_cst = itertools.product(fragment_paths, cst_files)
    # Match application does not work with -l option, has to be started individually for each fragment
    cmds = [f"{os.path.join(ROSETTA_BIN_PATH, match_application)} {' '.join(match_flags)} -s {frag} -match:geometric_constraint_file {cst}" for frag, cst in frag_cst]
    os.makedirs(output_dir, exist_ok=True)
    jobstarter.start(cmds=cmds, jobname="matching", output_path=output_dir)

def vectorize(atomslist): #vector is des. var name; ...maybe make list of vector,atom name tuples..
    vecs = []
    for i in atomslist:
        vector = i.get_vector()
        vecs.append(vector)
...
def calculate_cst_params(res_atoms, lig_atoms):
    cst = {}
    cst["distance"] = float(res_atoms[0] - lig_atoms[0])
    res_vectors = vectorize(res_atoms)
    lig_vectors = vectorize(lig_atoms)
    cst["angleA"] = dg(vectors.calc_angle(lig_vectors[1],lig_vectors[0],res_vectors[0]))
    cst["angleB"] = dg(vectors.calc_angle(lig_vectors[0],res_vectors[0],res_vectors[1]))
    cst["dihedralA"] = dg(vectors.calc_dihedral(lig_vectors[2],lig_vectors[1],lig_vectors[0],res_vectors[0]))
    cst["dihedralB"] = dg(vectors.calc_dihedral(lig_vectors[0],res_vectors[0],res_vectors[1],res_vectors[2]))
    cst["dihedralAB"] = dg(vectors.calc_dihedral(lig_vectors[1],lig_vectors[0],res_vectors[0],res_vectors[1]))
    cst = {key: round(value, 2) for key, value in cst.items()}
    return cst

def write_match_cst_file(cst:dict, res_atoms:list, lig_atoms:list, res_id:str, lig_id:str, path:str):
    cst_string = f'''CST::BEGIN 
  TEMPLATE:: ATOM_MAP: 1 atom_name: {lig_atoms[0]} {lig_atoms[1]} {lig_atoms[2]} 
  TEMPLATE:: ATOM_MAP: 1 residue3: {lig_id} 
  TEMPLATE:: ATOM_MAP: 2 atom_name: {res_atoms[0]} {res_atoms[1]} {res_atoms[2]} 
  TEMPLATE:: ATOM_MAP: 2 residue3: {res_id} 
  CONSTRAINT:: distanceAB: {cst["distance"]} 0.00 100.00 1 0 
  CONSTRAINT:: angle_A: {cst["angleA"]} 0.00 100.00 360.00 0 
  CONSTRAINT:: angle_B: {cst["angleB"]} 0.00 100.00 360.00 0 
  CONSTRAINT:: torsion_A: {cst["dihedralA"]} 0.00 100.00 360.00 0 
  CONSTRAINT:: torsion_B: {cst["dihedralB"]} 0.00 100.00 360.00 0 
  CONSTRAINT:: torsion_AB: {cst["dihedralAB"]} 0.00 100.00 360.00 0 
CST::END'''
    with open(path, "w+") as f:
        f.write(cst_string)

def create_match_cst_files(res_atoms, lig_atoms, res_id:str, path:str):
    cst = calculate_cst_params(res_atoms, lig_atoms)
    lig_id = lig_atoms[0].parent.get_resname()
    # do not take ids from res_atoms directly (because of equivalent functional groups!)
    atoms = func_groups()[res_id]
    write_match_cst_file(cst, atoms, [atom.id for atom in lig_atoms], res_id, lig_id, path)


def determine_closest_atoms(ligand_chain: Chain, residue: Residue, target_atoms: list[str]) -> list[Atom]:
    """
    Find the 3 closest atoms in the residue of chain_A with the smallest average distance to 
    specified atoms in residue_X.

    Parameters:
        chain_A (Chain): The chain containing residues to compare.
        residue_X (Residue): The reference residue with target atoms.
        target_atoms (List[str]): List of atom names to consider in residue_X (e.g., ["CA", "CB"]).
        
    Returns:
        List[Atom]: The 3 atoms from the residue with the smallest average distance to residue_X.
    """
    
    # Filter the atoms in residue_X to include only the target atoms
    target_atoms_in_X = [atom for atom in residue if atom.id in target_atoms]
    
    smallest_avg_distance = float('inf')
    closest_atoms = []
    
    for residue in ligand_chain.get_residues():
        # Collect distances between atoms in the current residue and target atoms in residue_X
        distances = []
        for atom in [h_atom for h_atom in residue.get_atoms() if not h_atom.element == "H"]:
            for target_atom in target_atoms_in_X:
                distance = atom - target_atom
                distances.append((distance, atom))
        
        # Sort by distance and get the 3 closest atoms in this residue
        sorted_distances = sorted(distances, key=lambda x: x[0])
        three_closest_atoms = [atom for _, atom in sorted_distances[:3]]
        
        # Calculate the average distance for these 3 closest atoms
        avg_distance = sum(d for d, _ in sorted_distances[:3]) / 3
        
        # Update if this residue has a smaller average distance for its 3 closest atoms
        if avg_distance < smallest_avg_distance:
            smallest_avg_distance = avg_distance
            closest_atoms = three_closest_atoms
...
def main(args):
    ...
    for resname in args.theozyme_resnums.split(","):
        ...
        for pos in frag_dict:
            ...
            if sc_clashes := list(set(sc_clashes)):
                log_and_print(f"Atoms involved in sidechain-ligand clashes: {sc_clashes}")
            ...
            for frag, df in zip(picked_frags, frag_dfs):
                ...
                covalent_bonds = args.covalent_bonds
    ...
    ######### DOUBLE ROTAMER FRAGMENTS ########
    as_res = {}
    os.makedirs(cst_dir := os.path.join(args.working_dir, "cst_files_single"), exist_ok=True)

    if args.jobstarter == "SbatchArray": jobstarter = SbatchArrayJobstarter(max_cores=args.cpus)
    elif args.jobstarter == "Local": jobstarter = LocalJobStarter(max_cores=args.cpus)

    if len([i for i in ligand.get_residues()]) > 1:
        raise RuntimeError("Multiple rotamer fragments currently only supported with single ligand!")
    os.makedirs(ligand_dir := os.path.join(args.working_dir, "matching", "ligand"), exist_ok=True)
    for lig in ligand:
        lig_id = lig.get_resname()
        ligand_pdbfile = save_structure_to_pdbfile(lig, lig_path:=os.path.abspath(os.path.join(ligand_dir, f"LG1.pdb")))
        ...
        if len(list(ligand.get_atoms())) > 2:
            ...
            cmd = f"{os.path.join(PROTFLOW_ENV, 'python')} {os.path.join(utils_dir, 'molfile_to_params.py')} -n {lig_name} -p {ligand_dir}/LG1 {lig_molfile} --keep-names --clobber --chain=Z"
            LocalJobStarter().start(cmds=[cmd], jobname="moltoparams", output_path=ligand_dir)
            params_path = lig_path.replace(".pdb", ".params")
        else:
            log_and_print(f"Ligand at {ligand_pdbfile} contains less than 3 atoms. No Rosetta Params file can be written for it.")

    log_and_print("Creating single .cst files...")
    for resname in args.theozyme_resnums.split(","):
        # import theozyme residue
        resnum, chain = split_pdb_numbering(resname)
        theozyme_residue = theozyme[0][chain][resnum]
        # determine atom of functional groups
        res_atom_names = func_groups()[theozyme_residue.get_resname()]
        # determine ligand atoms closest to functional groups
        lig_atoms = determine_closest_atoms(ligand, theozyme_residue, res_atom_names)
        res_atoms = [theozyme_residue[res_atom_names[0]], theozyme_residue[res_atom_names[1]], theozyme_residue[res_atom_names[2]]]
        if args.add_equivalent_func_groups:
            residue_identities = identify_residues_with_equivalent_func_groups(theozyme_residue)
        # write cst files for all residues
        cst_paths = []
        for res in residue_identities:
            create_match_cst_files(res_atoms, lig_atoms, res, path := os.path.join(cst_dir, f"{chain}{resnum}_{res}.cst"))
            cst_paths.append(path)
        as_res[f"{chain}{resnum}"] = {"ids": residue_identities, "residue": theozyme_residue, "cst_paths": cst_paths}
    
    log_and_print("Creating multi .cst files...")
    os.makedirs(cst_dir_multi := os.path.join(args.working_dir, "cst_files_multi"), exist_ok=True)
    # create every combination of theozyme residues
    multi_paths = []
    for pos1, pos2 in itertools.combinations(as_res.keys(), 2):
        # create every combination of cst files
        for path1, path2 in itertools.product(as_res[pos1]["cst_paths"], as_res[pos2]["cst_paths"]):
            idx1 = as_res[pos1]["cst_paths"].index(path1)
            idx2 = as_res[pos2]["cst_paths"].index(path2)
            res1 = as_res[pos1]["ids"][idx1]
            res2 = as_res[pos2]["ids"][idx2]
            with open(path1, 'r') as f:
                content1 = f.read()
            with open(path2, 'r') as f:
                content2 = f.read()
            with open(path := os.path.join(cst_dir_multi, f"{pos1}_{res1}_{pos2}_{res2}.cst"), "w") as f:
                f.write(content1 + '\n\n' + content2)
            multi_paths.append(path)
    run_matcher(cst_files=multi_paths, lig_name=lig_id, output_dir=os.path.join(args.working_dir, "matches"), match_application=args.match_application, params_path=params_path, database_dir=database_dir, jobstarter=jobstarter)
    


    # create multi cst files by generating every possible combination 

    # run match for every cst file

    # collect all matches

    # score all matches according to backbone score

    # extract chi angles and backbone angles and score rotamers
...
if __name__ == "__main__":
    ...
    argparser.add_argument("--match_application", type=str, default="match.linuxgccrelease", help="Rosetta Match application in ROSETTA_BIN folder")
